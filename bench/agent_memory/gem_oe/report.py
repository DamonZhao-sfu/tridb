...
def read_cell(cell: Path, ref: dict[str, dict[str, float]]) -> dict[str, Any] | None:
    ...
    for alpha in THRESHOLDS:
        ...
        reached[f"t{int(alpha * 100)}"] = (
            next((i for i, v in enumerate(curve) if v >= target), None)
            if r.get("gap", 0) > 0
            else None
        )
    ...
    freq = meta.get("injection_frequency")
    arm = meta["arm"]
    if arm in ("nocontext", "none"):
        label = arm
    else:
        label = arm
        if rate is not None:
            label += f"@{rate:.0%}"
        # arXiv:2606.29823's p. Kept in the label because two cells of the same arm
        # and the same injection size are otherwise indistinguishable.
        if freq is not None and freq != 1.0:
            label += f"/p{freq:g}"
        if meta.get("physical_plan"):
            label += f"/{meta['physical_plan']}"
    # Iterations RAN, from the receipt -- not lines in the trace. A failed or
    # discarded iteration produces no trace row, so trace length undercounts by
    # however many the run threw away, and three heilbronn_triangle cells that
    # finished all 40 iterations were dropped as "short" on trace lengths of
    # 35, 32 and 30.
    ran = meta.get("iterations_traced") or len(curve)
    return {
        "task": task.split(":")[-1],
        "arm": label,
        "base_arm": arm,
        "injection_frequency": freq,
        "physical_plan": meta.get("physical_plan"),
        "iters": ran,
        "traced": len(curve),
        ...
        "corpus_best": r.get("best"),
        "seed": r.get("seed"),
        "normalized_auc": (
            round(
                statistics.fmean(
                    max(0.0, min(1.0, (value - r.get("seed", 0.0)) / r["gap"]))
                    for value in curve[:40]
                ),
                6,
            )
            if curve and r.get("gap", 0.0) > 0
            else None
        ),
        **reached,
        **lat,
...
def main(argv: list[str] | None = None) -> int:
    ...
    cells = sorted(
        {c for pat in CELL_GLOB for c in args.matrix.rglob(pat) if c.is_dir()}
    )
    ...
    # Grouped by task, because the primary metric is now the RAW score and raw
    # scores are only comparable within a task -- circle_packing's 2.6 and
    # heilbronn_triangle's 0.98 are different units. Tasks are ordered by
    # headroom so the discriminating ones come first, but the number read off
    # the table is the evaluator's own score, not a normalisation of it.
    rows.sort(
        key=lambda r: (-(r["headroom"] or 0), r["task"], arm_order.get(r["arm"], 9))
    )

    print("表 1 — Quality（主指标 = best：evaluator 给出的原始分数）")
    print(
        f"{'task':<22}{'arm':<20}{'best':>9}{'起点':>9}{'语料最佳':>10}"
        f"{'Δ起点':>9}{'已达':>7}{'t25':>5}{'t50':>5}{'t75':>5}{'t90':>5}"
        f"{'0分':>5}{'不同':>5}{'抄%':>6}"
    )
    for r in rows:
        ...
        raw = r["best"] if r["best"] is not None else 0.0
        delta = raw - (r["seed"] or 0.0)
        print(
            f"{r['task']:<22}{low}{r['arm']:<19}{raw:>9.4f}"
            f"{(r['seed'] or 0):>9.4f}{(r['corpus_best'] or 0):>10.4f}"
            f"{delta:>+9.4f}{(f'{pct:.0%}' if pct is not None else '--'):>7}"
            + "".join(
                f"{(r[k] if r[k] is not None else '--'):>5}"
                for k in ("t25", "t50", "t75", "t90")
            )
            + f"{r['zero']:>5}{r['distinct']:>5}{r['copy_rate'] * 100:>5.0f}%"
        )
    print(
        "\n  best = evaluator 原始分数（主指标）。起点 = 种子程序的分数，"
        "语料最佳 = 该题在 EvoTrace 语料里出现过的最高分。"
    )
    print("  已达 = (best - 起点) / (语料最佳 - 起点)，仅用于跨题对比，非主指标。")
    print(
        f"  * 标记：余量 < {HEADROOM_FLOOR:.0%}，该题上任何 arm 差异都落在 "
        "evaluator 自身的库版本漂移内，不可解读"
    )
    ...
    if lat_rows:
        print("\n表 2 — 检索延迟(仅有检索的 arm)")
        print(
            f"{'task':<22}{'arm':<20}{'first_row':>11}{'total':>9}"
            f"{'ann':>8}{'traverse':>10}{'filter':>9}"
        )
        for r in lat_rows:
            print(
                f"{r['task']:<22}{r['arm']:<14}{r['first_row_ms']:>10.1f}ms"
