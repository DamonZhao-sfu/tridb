...
double calculate_beam_score(const ScoreComponents& scores, int K_total, const Board& current_board_state) {
    ...
    // Primary objective: maximize S (largest tree size)
    // Secondary: minimize number of components (connectivity)
    // Tertiary: minimize K (number of moves)
    // Quaternary: keep empty near corner (N-1, N-1)
    
    double score_val = (double)S * 1e9;
    
    // Strong penalty for multiple components - encourages building one big tree
    int nc = scores.num_components;
    if (nc > 1) {
        score_val -= (double)(nc - 1) * 8e8;
    } else if (nc == 0 && target_size > 0) {
        score_val -= (double)target_size * 8e8;
    }
    
    // Strong bonus when very close to full tree
    int remaining = target_size - S;
    if (remaining <= 3 && nc == 1) {
        score_val += (double)(4 - remaining) * 5e8;
    } else if (remaining <= 5 && nc == 1) {
        score_val += (double)(6 - remaining) * 1e8;
    }
    
    // Moderate penalty for move count
    score_val -= (double)K_total * 2.0;
    ...
    score_val -= dist_empty_to_corner * 1.0;
