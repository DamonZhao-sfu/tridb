...
static constexpr std::array<int, MAX_LOOKAHEAD_DEPTH> NUM_SAMPLES_CONFIG = {15, 8};
...
struct GameState {
    ...
    std::pair<int, int> find_pth_empty_cell(int p_1_indexed) const {
        ...
        for (int r_idx = 0; r_idx < GRID_SIZE; ++r_idx) {
            const auto& row = board[r_idx];
            for (int c_idx = 0; c_idx < GRID_SIZE; ++c_idx) {
                if (row[c_idx] == 0) { 
                    count++;
                    if (count == p_1_indexed) return {r_idx, c_idx};
                }
            }
        }
        return {-1, -1}; 
    ...
    double calculate_distance_penalty_CoM() const {
        ...
        for (const auto& candy : candies_list) {
            int fl = candy.flavor;
            if (counts[fl] > 1) {
                double com_r = sum_r[fl] / counts[fl];
                double com_c = sum_c[fl] / counts[fl];
                double dr = candy.r - com_r;
                double dc = candy.c - com_c;
                total_penalty += dr * dr + dc * dc;
            }
        }
    ...
    double calculate_region_penalty() const {
        ...
        for (const auto& candy : candies_list) {
            ...
            int min_c = range.first, max_c = range.second;
            if (min_c > max_c) continue;
            int dist = 0;
            if (candy.c < min_c) dist = min_c - candy.c;
            else if (candy.c > max_c) dist = candy.c - max_c;
            penalty += dist * dist;
        }
        return penalty;
    }
    ...
    double evaluate() const { 
        ...
        double A = 25.0 + 2.0 * t;
        double B = std::max(0.0, 150.0 - 1.5 * t);
        double C = std::max(5.0, 40.0 - 0.2 * t);
        double D = 8.0 + 0.4 * t;
                
        return A * sum_sq_comp 
             - B * dist_penalty_com 
             - C * region_penalty_val
             + D * edge_bonus_val;
    }
};
...
double eval_lookahead(const GameState& state_after_tilt, int turn_T_of_candy_just_processed, int depth_remaining) {
    ...
    // Precompute empty cell positions for faster sampling
    std::array<std::pair<int,int>, GRID_SIZE*GRID_SIZE> empty_cells;
    int empty_count = 0;
    for (int r = 0; r < GRID_SIZE; ++r) {
        for (int c = 0; c < GRID_SIZE; ++c) {
            if (state_after_tilt.board[r][c] == 0) {
                empty_cells[empty_count++] = {r, c};
            }
        }
    }
    (void)num_empty; // use empty_count instead

    double sum_over_sampled_placements = 0.0;
    for (int s = 0; s < actual_num_samples; ++s) {
        int idx;
        if (actual_num_samples == empty_count) { 
            idx = s; 
        } else { 
            idx = rng.uniform_int(0, empty_count - 1); 
        }
        
        int pr = empty_cells[idx].first;
        int pc = empty_cells[idx].second;
        
        GameState S_after_placement = state_after_tilt; 
        S_after_placement.place_candy(pr, pc, next_candy_flavor);
