...
double calculate_beam_score(const ScoreComponents& scores, int K_total, const Board& current_board_state) {
    ...
    int total_tiles = N_actual * N_actual - 1;
    ...
    double W_S = 1e9; 
    double W_NC = W_S * 0.5; // Reduced component penalty to allow more exploration
    double W_K = 1.0; 
    
    double score_val = (double)S * W_S;
    if (scores.num_components > 1) { 
         score_val -= (double)(scores.num_components - 1) * W_NC; 
    } else if (scores.num_components == 0 && total_tiles > 0) {
         score_val -= (double)total_tiles * W_NC; 
    }

    // Bonus for being very close to a full tree and connected
    if (scores.num_components == 1 && S < total_tiles) {
        int deficit = total_tiles - S;
        if (deficit <= 3) score_val += W_S * 0.6; // Stronger bonus to encourage the last few moves
        else if (deficit <= 5) score_val += W_S * 0.3;
        else if (deficit <= 8) score_val += W_S * 0.1;
        else if (deficit <= 12) score_val += W_S * 0.05;
    }
...
const int TIME_LIMIT_MS_SLACK_CONST = 250; // Give more time to beam search while staying safe
...
int main(int /*argc*/, char** /*argv*/) {
    ...
    int K_initial_empty_moves = 0;

    // Adaptive time limit
    auto time_after_init = std::chrono::steady_clock::now();
    long long elapsed_init_ms = std::chrono::duration_cast<std::chrono::milliseconds>(time_after_init - T_START_CHRONO_MAIN).count();
    TIME_LIMIT_MS_EFFECTIVE_MAIN = 2950 - elapsed_init_ms - TIME_LIMIT_MS_SLACK_CONST;
    ...
    int beam_width; 
    float elite_ratio = 0.35f; // Moderate elite ratio – keep more promising candidates
    int stochastic_sample_pool_factor = 4; // Larger stochastic pool for better exploration

    if (N_actual <= 5) { beam_width = 2000;}
    else if (N_actual == 6) { beam_width = 1500;}
    else if (N_actual == 7) { beam_width = 1000;}
    else if (N_actual == 8) { beam_width = 700;}
    else if (N_actual == 9) { beam_width = 700;} // Better exploration for 9x9
    else { beam_width = 500;} // N=10, moderate – better coverage for hardest case
    ...
    for (int k_beam_iter = 0; K_initial_empty_moves + k_beam_iter < T_param; ++k_beam_iter) {
        ...
        for (const auto& current_state_in_beam : current_beam) {
            ...
            for (int move_dir_idx = 0; move_dir_idx < 4; ++move_dir_idx) { 
                // Allow all moves including reversals - the min_K cache handles dedup
